...
async function fetchFromApi(endpoint: string): Promise<ApiResponse<any>> {
  try {
    ...
    throw new Error(
      `Failed to fetch data from ${endpoint}: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
...
function getNextFixture(
  ...
  const nextFixture = fixtures.find(
    (f) =>
      (f.team_h === playerTeamId || f.team_a === playerTeamId) && !f.finished && f.event !== null
  );
...
export function calculateBuyRecommendation(
  ...
  try {
    ...
    const performanceScore = Math.min(
      100,
      pointsPerGame * 8 + (influence / 100) * 4 + (creativity / 100) * 4 + (threat / 100) * 4
    );
  ...
  try {
    ...
    const allPredictions: PlayerPrediction[] = players
      .filter((player) => {
        ...
        return (
          (minutes > 0 || chanceOfPlaying > 0) && form > 0 && pointsPerGame > 0 && minutes >= 270
        );
      })
      .map((player) => {
        const form = calculateForm(player);
    ...
    while (selectedPredictions.length < 25 && index < sortedPredictions.length) {
      ...
      if (
        !selectedTeams.has(prediction.player.team) ||
